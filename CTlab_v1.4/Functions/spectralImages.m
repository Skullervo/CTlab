...
%         ax2.Title.String = sprintf('Spectral Reconstruction %d, Energy range: %d keV - %d keV',j,energyWindows(j),energyWindows(j+1));
%         image = spectralImages_logsum(:,:,j);
% %         [rec, I1, I2, h] = filteredBackprojection2(image,W,ax2,ax4);
%         [rec,I1,I2,h]=least_squares(fig,W,image,iterations,ax2,ax4)
%         minInt1(j) = I1;
%         maxInt2(j) = I2;
%         spectralIm(:,:,j)=rec;
%         
%         R=rectangle('Position', [energyWindows(j), 0, energyWindows(j+1)-energyWindows(j), 100000],'FaceColor', [0, 0.5, 0.5, 0.7], 'EdgeColor', [1, 0, 0, 0.7],'LineWidth', 1.5,'Parent',ax3);
%         
%         for k=1:length(energyWindows)
%             
%             xl=xline(energyWindows(k),':','Parent',ax3);
